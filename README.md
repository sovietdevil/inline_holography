...
$$-\frac{\hbar}{\mathrm{i}} \frac{\partial}{\partial t} \Psi(\mathbf{R}, t)=H \Psi(\mathbf{R}, t)=\left[-\frac{\hbar^2}{2 m} \Delta_\mathbf{R}-e U(\boldsymbol{R}, t)] \Psi(\mathbf{R},t)$$
...
Therefore,
$$\frac{\partial}{\partial z}\Psi(\mathbf{R}, z)=\frac{i}{2k}\left(\Delta_{\mathbf{R}}+\frac{2me}{\hbar^2}U(\mathbf{R},z))\Psi(\mathbf{R}, z)$$
...
$$\frac{\partial}{\partial z}\Psi(\mathbf{R}, z)=\exp \left\{\int_{0}^{h}\left[\frac{i}{2k}\Delta_\mathbf{R}+\frac{ime}{k\hbar^2}U(\mathbf{R},z)]dz\}\Psi(\mathbf{R}, z)$$
...
Here $v_{\Delta z}(x, y, z)=\int_z^{z+\Delta z} V\left(x, y, z^{\prime}) \mathrm{d} z^{\prime}$
...
$$
Here $t(x, y, z)=\exp \left[i \sigma \int_z^{z+\Delta z} V\left(x, y, z^{\prime}) \mathrm{d} z^{\prime}]$
...
Thus in real space,
$$p(x, y, \Delta z)=\mathrm{FT}^{-1}[P(k, \Delta z)]=\frac{1}{\mathrm{i} \lambda \Delta z} \exp \left[\frac{\mathrm{i} \pi}{\lambda \Delta z}\left(x^2+y^2)]$$
such that
$$\psi(x, y, z+\Delta z)=p(x, y, \Delta z) \otimes[t(x, y, z) \psi(x, y, z)]+\mathscr{O}\left(\Delta z^2)$$
in numerical computation, it is noted as
$$\psi_{n+1}(x, y)=p_n\left(x, y, \Delta z_n) \otimes\left[t_n(x, y) \psi_n(x, y)]+\mathscr{O}\left(\Delta z^2)$$
which can also be expressed as
$$\psi_{n+1}(x, y)=t_n(x, y)\left[p_n\left(x, y, \Delta z_n) \otimes \psi_n(x, y)]+\mathscr{O}\left(\Delta z^2)$$
...
$$\Psi(\boldsymbol{R}, z)=\sum_n C_n \Phi_n(\boldsymbol{R}) \exp \left\{-\mathrm{i} \frac{E_n}{2E_{0}} k z\}$$
...
In the column:
$$\Psi(\mathbf{R},z)=1+\sum_{nm}2c_{nm}sin\left(\frac{E_{nm}}{E_0}\frac{k_z}{4}z)\psi_{nm}(\mathbf{R})\times \exp\left\{-i\left[\frac{\pi}{2} +\frac{E_{nm}}{E_0}\frac{k_z}{4}z]\}$$
...
and
$$-\left\{\frac{\hbar^2}{2 \mu}\left(\frac{\partial^2}{\partial \rho^2}+\frac{1}{\rho} \frac{\partial}{\partial \rho}-\frac{m^2}{\rho^2})+e U(\rho)\}R_{n m}(\rho)=E_{n m} R_{n m}(\rho)$$

When only 1s state is considered, the expression is:
$$\psi(\mathbf{r}, z)=1+c_s \varphi_s(\mathbf{r}-\boldsymbol{\beta})\left[\exp \left(-i \frac{E_s}{2E_0} k z)-1]$$
Here $\varphi_s(\mathbf{r})=\frac{1}{a \sqrt{2 \pi}} \exp \left(-\frac{r^2}{4 a^2})$, $c_s=2 \sqrt{2 \pi} a$.
...
$$\exp(\hat{H}t)\left(\frac{d\theta}{dt}+\hat{H}\theta)=\exp(\hat{H}t)(\hat{H}+W)\theta$$
